...
class core_test : public cuDevice
{
public:
	core_test(const cuPlatform & pf, size_t d) : cuDevice(pf, d, true) {}
	...
	bool run()
	{
		...
		cu_mem da = _createBuffer(CU_MEM_READ_ONLY, bytes);
		cu_mem db = _createBuffer(CU_MEM_READ_ONLY, bytes);
		cu_mem dout = _createBuffer(CU_MEM_READ_WRITE, bytes);
		cu_mem dout2 = _createBuffer(CU_MEM_READ_WRITE, bytes);

		cu_kernel kMont = _createKernel("k_mont");
		cu_kernel kRound = _createKernel("k_roundtrip");
		cu_kernel kAddsub = _createKernel("k_addsub");
		...
		for (const Prime & pr : primes)
		{
			...
			uint32_t n = N;
			_setKernelArg(kMont, 0, sizeof(cu_mem), &da);
			_setKernelArg(kMont, 1, sizeof(cu_mem), &db);
			_setKernelArg(kMont, 2, sizeof(cu_mem), &dout);
			...
			_setKernelArg(kRound, 0, sizeof(cu_mem), &da);
			_setKernelArg(kRound, 1, sizeof(cu_mem), &dout2);
			...
			// --- addmod/submod ---
			_setKernelArg(kAddsub, 0, sizeof(cu_mem), &da);
			_setKernelArg(kAddsub, 1, sizeof(cu_mem), &db);
			_setKernelArg(kAddsub, 2, sizeof(cu_mem), &dout);
			_setKernelArg(kAddsub, 3, sizeof(cu_mem), &dout2);

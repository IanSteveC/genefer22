...
private:
	...
	static transform * create_cuda(const uint32_t b, const uint32_t n, const bool isBoinc, const size_t device, const size_t num_regs,
								   const bool verbose);
#elif defined(HIP)
	static transform * create_hip(const uint32_t b, const uint32_t n, const bool isBoinc, const size_t device, const size_t num_regs,
								  const bool verbose);
#else
...
public:
	...
	{
		// gpu_device_num: BOINC-assigned GPU ordinal (>= 0), else use the -d device.
...
#else
		transform * const pTransform = transform::create_hip(b, n, isBoinc, dev, num_regs, verbose);
#endif
		if (pTransform == nullptr) throw std::runtime_error("GPU device not found");
		return pTransform;
	}
#else
	static transform * create_gpu(const uint32_t b, const uint32_t n, const bool isBoinc, const size_t device, const size_t num_regs,

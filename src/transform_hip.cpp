...
----
HIP backend factory. Counterpart of transform_ocl.cpp: same RNS_SIZE/is32 NTT-limit
thresholds, but instantiates the (shared) transformGPUs template against the CUDA
device backend (hip.h, selected by -DHIP in transformGPU.h) and takes a CUDA device
ordinal instead of (device ordinal).
*/
...
transform * transform::create_hip(const uint32_t b, const uint32_t n, const bool isBoinc, const size_t device, const size_t num_regs,
								   const bool verbose)
{
